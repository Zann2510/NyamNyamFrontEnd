...
export default function CheckoutPage() {
  ...
  const handleUploadProof = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // ✅ Gunakan endpoint khusus customer
      const res = await api.post('/upload/payment-proof', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      const imageUrl = res.data.data.url;
      console.log('Upload success, image URL:', res.data.data.url);
    ...
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-5">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm">
          <label className="font-semibold block mb-1">Alamat Pengiriman <span className="text-red-500">*</span></label>
          <textarea required rows={3} className="w-full border rounded-lg p-2" placeholder="Jl. Sudirman No. 45, Jakarta Selatan" value={address} onChange={e => setAddress(e.target.value)} />
        </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <label key={method.value} className="flex items-center gap-2">
                <input type="radio" name="paymentMethod" value={method.value} checked={paymentMethod === method.value} onChange={e => setPaymentMethod(e.target.value)} className="text-orange-500" />
        ...
        {paymentMethod === 'QRIS' && (
          <div className="bg-white p-4 rounded-xl shadow-sm text-center">
            ...
            <div className="flex justify-center">
              <img src="/images/qris.png" alt="QRIS Code" className="w-48 h-48 object-contain border rounded-lg" />
            </div>
        ...
        {paymentMethod === 'CARD' && (
          <div className="bg-white p-4 rounded-xl shadow-sm">
            ...
            <select className="w-full border rounded-lg p-2 mb-3" value={selectedBank.name} onChange={e => setSelectedBank(banks.find(b => b.name === e.target.value) || banks[0])}>
        ...
        {paymentMethod !== 'CASH' && (
          <div className="bg-white p-4 rounded-xl shadow-sm">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`text-sm font-mono ${timeLeft < 30 ? 'text-red-500' : 'text-gray-600'}`}>Sisa waktu: {formatTime(timeLeft)}</span>
            </div>
            {paymentProofUrl ? (
              <div className="relative inline-block">
                <img src={paymentProofUrl} alt="Bukti" className="h-24 rounded-md" />
                <button type="button" onClick={() => setPaymentProofUrl('')} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 text-xs">✕</button>
              </div>
            ) : (
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm">
          ...
          {items.map(item => <div key={item.productId} className="flex justify-between text-sm py-1"><span>{item.name} x{item.quantity}</span><span>{formatRupiah(item.price * item.quantity)}</span></div>)}
          <hr className="my-2" />
        ...
        <button type="submit" disabled={loading || (paymentMethod !== 'CASH' && !paymentProofUrl) || (paymentMethod !== 'CASH' && timeLeft === 0)} className="w-full bg-orange-500 text-white py-3 rounded-full font-semibold disabled:opacity-50">
